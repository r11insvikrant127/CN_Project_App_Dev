...
class _DeviceVerificationScreenState extends State<DeviceVerificationScreen> {
  ...
  Future<void> _authenticateWithFingerprint() async {
    print("\n🔵 START: Fingerprint authentication started");
  ...
  Future<void> _getDeviceId() async {
    try {
      ...
      print("🟦 Device ID built: $_deviceId");
  ...
  Future<bool> _authenticateWithBackend(String token) async {
    print("🟦 START: _authenticateWithBackend() - Token verification");
  ...
  Future<void> _verifyAndSetupWithBackend() async {
    print("🟦 START: _verifyAndSetupWithBackend() - Device verification");
    ...
    try {
      // 🔵 DEBUGGING PRINTS — These are the MOST IMPORTANT
      final url = "$kBaseUrl/api/verify-device";
      ...
      if (response.statusCode == 200) {
        ...
        if (data['verified'] == true) {
          ...
          await BiometricAuthService.storeSessionTokenForRole(
              'device_verification', sessionToken);
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => RoleSelectionScreen(authMethod: 'device'),
            ),
          );
        } else {
          setState(() {
            ...
            _verificationFailed = true;
            _showAuthOptions = true;
            _statusMessage = data['message'] ?? 'Device not registered.';
          });
        }
      } else {
        setState(() {
      ...
      setState(() {
        ...
        _showAuthOptions = true;
        _statusMessage =
            'Network error. Backend may be asleep. Try again in 5 seconds.';
      });
    }
  }
  ...
  void _retryVerification() {
    print("🟦 _retryVerification() - Resetting verification state");
