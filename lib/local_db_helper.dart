...
class LocalDBHelper {
  ...
  Future<void> _createTables(Database db, int version) async {
    ...
    await db.execute('''
      CREATE TABLE $_tableSecurityScans(
        ...
        $_columnEventId TEXT NOT NULL UNIQUE,
  ...
  Future<int> saveSecurityScan({
    ...
    try {
      ...
      final id = await db.insert(
        _tableSecurityScans,
        {
          _columnEventId: eventId ?? _uuid.v4(),
      ...
      print('🔍 DEBUG: Saved security scan - '
          'Roll: $rollNo, Action: $action, Event ID: ${eventId ?? "generated"}');
      return id;
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getRecentSecurityScans(
      {int limit = 10}) async {
    try {
      final db = await database;
      final result = await db.query(
  ...
  Future<int> getPendingRecordsCount() async {
    try {
      ...
      final securityCount = Sqflite.firstIntValue(await db.rawQuery(
              'SELECT COUNT(*) FROM $_tableSecurityScans WHERE $_columnIsSynced = 0')) ??
          0;

      final canteenCount = Sqflite.firstIntValue(await db.rawQuery(
              'SELECT COUNT(*) FROM $_tableCanteenVisits WHERE $_columnIsSynced = 0')) ??
          0;

      final total = securityCount + canteenCount;
      print(
          '🔍 DEBUG: Pending records - Security: $securityCount, Canteen: $canteenCount');
      return total;
    } catch (e) {
  ...
  Future<bool> securityScanExists(
      String rollNo, String action, DateTime timestamp) async {
    try {
      final db = await database;
      final result = await db.query(
        _tableSecurityScans,
        where:
            '$_columnRollNo = ? AND $_columnAction = ? AND $_columnTimestamp = ?',
